...
<div class="row justify-content-center">
	<form action="php/portion_proccess.php" method="POST">
		<?php if (isset($_GET['edit'])) {
		}else{ ?>
		<div class="col col-sm-6">
		<div class="form-group">
			<label>: الشهر</label>
			<input type="hidden" name="p_month" value="<?php echo $the_month ?>">
		...
		<div class="form-group">
			<label>: العام </label>
			<input type="hidden" name="p_year" value="<?php echo $the_year ?>">
...
<div class="row justify-content-center">
	...
	$update = true;
	$r = $_GET['r'];
	$result=$conn->query("SELECT DISTINCT marakez.ID, marakez.mname FROM marakez INNER JOIN portions ON marakez.ID = portions.p_markaz WHERE portions.p_year='$id' AND portions.p_month='$id2'  ORDER BY ID DESC") or die($conn->error());
...
<div class="row justify-content-center">
	<table class="table cen1">
		...
		<?php 
			$result=$conn->query("SELECT DISTINCT items.ID, items.i_name FROM items INNER JOIN portions ON items.ID = portions.p_item WHERE portions.p_year='$id' AND portions.p_month='$id2'") or die($conn->error());
			while ($row = $result->fetch_assoc()) { ?>
			<tr class="me">
			<?php
			$result2 = $conn->query("SELECT DISTINCT marakez.ID, marakez.mname FROM marakez INNER JOIN portions ON marakez.ID = portions.p_markaz WHERE portions.p_year='$id' AND portions.p_month='$id2' ORDER BY ID DESC") or die($conn->error());
			$x = $t * $num1;
			while ($row2 = $result2->fetch_assoc()) {
			$result3 = $conn->query("SELECT * FROM portions WHERE p_year='$id' AND p_month='$id2' AND p_markaz='".$row2['ID']."' AND p_item='".$row['ID']."'") or die($conn->error());
			$row3 = $result3->fetch_assoc(); ?>
			<td><input type="hidden" name="id1[]" value="<?php echo $row3['ID']; ?>"><input type="" name="p_price1[]" tabindex="<?php echo $x; $x--?>" class="form-control cen2" ondrop="return false;" onpaste="return false;" onkeypress="return event.charCode>=48 && event.charCode<=57" autocomplete="off" value="<?php echo $row3['p_price']; ?>" style="width: 100%;padding: 0;"></td>
			<td class="rig1"><input type="" tabindex="<?php echo $x; $x--?>" class="form-control cen2" name="p_quantity1[]" autocomplete="off" ondrop="return false;" onpaste="return false;" onkeypress="return event.charCode>=48 && event.charCode<=57 || event.charCode==46" value="<?php echo $row3['p_quantity']; ?>" style="width: 100%;padding: 0;"></td>
			<?php $t++; } ?>
	...
	$result2 = $conn->query("SELECT * FROM items") or die($conn->error());
	?>
	<table class="table cen1">
		<thead>
		...
		<?php while ($row2 = $result2->fetch_assoc()){ ?>
			...
			$x = $t * $num1;
			while ($row = $result->fetch_assoc()) {	?>
			<td><input type="hidden" name="p_markaz[]" value="<?php echo $row['ID']; ?>"><input type="" name="p_price[]" tabindex="<?php echo $x; $x--?>" class="form-control cen2" ondrop="return false;" onpaste="return false;" onkeypress="return event.charCode>=48 && event.charCode<=57" autocomplete="off" style="width: 100%;padding: 0;"></td>
			<td class="rig1"><input type="" class="form-control cen2" tabindex="<?php echo $x; $x--?>" name="p_quantity[]" autocomplete="off" ondrop="return false;" onpaste="return false;" onkeypress="return event.charCode>=48 && event.charCode<=57 || event.charCode==46" style="width: 100%;padding: 0;"><input type="hidden" name="p_item[]" value="<?php echo $row2['ID']; ?>"></td>
			<?php $t++; } ?>
