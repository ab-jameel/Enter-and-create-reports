...
<head>
	...
	<title>اعتماد تقارير التعليم</title>
<?php 
session_start();
...
<?php
$result = $conn->query("SELECT rok.*, treports.ID as ID2, treports.t_name as t_name FROM rok INNER JOIN treports ON rok.r_type = treports.ID WHERE k_type=0 AND (r_type = 5 or r_type = 3 or r_type = 4) ORDER BY k_year ASC, r_type ASC, k_month ASC") or die($conn->error());
?>
<br>
