...
if ($learn == 1 and $lreports == 1){

$result2 = $conn->query("SELECT premissions.*, pages.p_name FROM premissions INNER JOIN pages ON premissions.p_page = pages.ID WHERE p_user=$id AND premissions.p_manage = 1") or die($conn->error());
while ($row2 = $result2->fetch_assoc()) {
if ($row2['p_name'] == 'مسيرات الرواتب'){
$salary = $row2['p_pdf'];
}
if ($row2['p_name'] == 'مصروفات المراكز'){
$expensesm = $row2['p_pdf'];
}
...
$result = $conn->query("SELECT rok.*, treports.t_name, report_months.m_name FROM rok INNER JOIN treports ON rok.r_type = treports.ID INNER JOIN report_months ON k_month = report_months.ID WHERE k_type=1 AND k_year='".$the_year."' AND (r_type = 5 or r_type = 3 or r_type = 4) ORDER BY k_year ASC, k_month ASC, r_type ASC") or die($conn->error());
} ?>
