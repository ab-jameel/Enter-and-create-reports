...
<style type="text/css">
	...
	<title>بنود الرواتب</title>

<?php
require_once 'php/sitems_proccess.php';
...
$result1 = $conn->query("SELECT * FROM premissions WHERE p_user = $id AND p_page = 31") or die($conn->error());
...
<br>
<?php
$result = $conn->query("SELECT * FROM sitems") or die($conn->error());
?>
<div class="container">
<div class="row justify-content-center">
	<form action="php/sitems_proccess.php" method="POST">
	...
	<div class="form-group">
		...
		<input autocomplete="off" tabindex="1" maxlength="25" type="text" name="i_name" class="form-control right1" style="height: 35px;" value="<?php echo $i_name ?>" placeholder="ادخل اسم البند">
	</div>
	</div>
	</form>
</div>
<div class="row justify-content-center">
	<table class="table right1">
		...
		<?php while ($row = $result->fetch_assoc()):
			$r1 = $conn->query("SELECT * FROM salary WHERE s_item = ".$row['ID']) or die($conn->error());
			$num = mysqli_num_rows($r1);
		?>
			<tr class="me">
				<td>
				<?php if ($num == 0 and $delete == 1){?>
					<a href="php/sitems_proccess.php?delete=<?php echo $row['ID']; ?>"><i class="fas fa-trash-alt i1"></i></a>
				<?php } ?>
				<?php if ($edit == 1){ ?>
				<a href="sitems.php?edit=<?php echo $row['ID']; ?>"><i class="fas fa-pencil-alt i1"></i></a>
				<?php } ?>
				</td>
