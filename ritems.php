...
<style type="text/css">
	...
	<title>بنود المصروفات التشغيلية</title>

<?php
require_once 'php/ritems_proccess.php';
...
$result1 = $conn->query("SELECT * FROM premissions WHERE p_user = $id AND p_page = 32") or die($conn->error());
...
<div class="row justify-content-center">
	<form action="php/ritems_proccess.php" method="POST">
...
<div class="row justify-content-center">
	<table class="table right1">
		...
		<?php while ($row = $result->fetch_assoc()):
			$r1 = $conn->query("SELECT * FROM expensesr WHERE r_item = ".$row['ID']) or die($conn->error());
			$num = mysqli_num_rows($r1);
		?>
			<tr class="me">
				<td>
				<?php if ($num == 0 and $delete == 1){ ?>
					<a href="php/ritems_proccess.php?delete=<?php echo $row['ID']; ?>"><i class="fas fa-trash-alt i1"></i></a>
				<?php } ?>
				<?php if ($edit == 1){ ?>
				<a href="ritems.php?edit=<?php echo $row['ID']; ?>"><i class="fas fa-pencil-alt i1"></i></a>
				<?php } ?>
				</td>
