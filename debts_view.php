...
<div style="margin-right: 125px;">
	...
	<p class="right1" style="font-size: 17.5px;">تقرير المديونيات</p>
...
<table class="table">
	<tr>
		<td class="hst" style="border-left-color: black;">الباقي</td>
		<td class="hst">اجمالي السداد</td>
		<td class="hst">ديون مدورة</td>
		<td class="hst">سعر الصرف</td>
		<td class="hst" rowspan="2" style="border-bottom-color: black; vertical-align: middle;">حركة الديون بالدولار</td>
	</tr>
	<tr>
		<td class="dst"><?php echo number_format($left); ?></td>
		<td class="dst"><?php echo number_format($d_total); ?></td>
		<td class="dst"><?php echo number_format($s_total); ?></td>
...
<table class="table">
	<tr class="th4">
		<td class="c230">رقم السند</td>
		<td class="c230">المسؤول</td>
		<td class="c230">البيان</td>
		<td class="c230">ما يعادل بالدولار</td>
		<td class="c230">المبلغ</td>
		<td class="c230">العملة</td>
		<td class="c230">خاص بمركز</td>
	...
	<?php while ($row = $result1->fetch_assoc()){ ?>
		<tr>
			...
			<td></td>
			<td>الرصيد المدور</td>
			<?php if ($row['b_currency'] == 'شلن'){ ?>
				<td><?php echo number_format($row['b_amount'] / $exchange); ?></td>
			<?php }else{ ?>
				<td></td>
			<?php } ?>
			<td><?php echo number_format($row['b_amount']); ?></td>
			<td><?php echo $row['b_currency']; ?></td>
			<td></td>
	...
	?>
		<tr>
			<td><?php echo $row2['d_number']; ?></td>
			<td><?php echo $row2['d_admin']; ?></td>
			<td><?php echo $row2['d_notice']; ?></td>
			<?php if ($row2['d_currency'] == 'شلن'){ ?>
				<td><?php echo number_format($row2['d_amount'] / $exchange); ?></td>
			<?php }else{ ?>
				<td></td>
			<?php } ?>
			<td><?php echo number_format($row2['d_amount']); ?></td>
			<td><?php echo $row2['d_currency']; ?></td>
			<td><?php echo $markaz['mname']; ?></td>
			<td><?php echo $row2['d_date']; ?></td>
		</tr>
	<?php } ?>
</table>
...
<div class="col col-sm-3">
	<div class="form-group">
		<a href="debts_report.php?y=<?php echo $_GET['y']; ?>" class="cancel" target="_blank">طباعة</a>
