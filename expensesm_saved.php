...
<?php
$result=$conn->query("SELECT DISTINCT marakez.ID, marakez.mname FROM marakez INNER JOIN expensesm ON marakez.ID = expensesm.e_markaz WHERE expensesm.e_year='$the_year' AND expensesm.e_month='$the_month' ORDER BY ID DESC") or die($conn->error());
?>
<div class="row justify-content-center">
	<table class="table cen1">
		...
		<?php 
			$result=$conn->query("SELECT DISTINCT eitems.ID, eitems.i_name, eitems.i_type FROM eitems INNER JOIN expensesm ON eitems.ID = expensesm.e_item WHERE expensesm.e_year='$the_year' AND expensesm.e_month='$the_month' ORDER BY i_type DESC") or die($conn->error());
			$result3 = $conn->query("SELECT DISTINCT eitems.ID, eitems.i_name, eitems.i_type FROM eitems INNER JOIN expensesm ON eitems.ID = expensesm.e_item WHERE expensesm.e_year='$the_year' AND expensesm.e_month='$the_month' AND i_type = 2") or die($conn->error());
			$result4 = $conn->query("SELECT DISTINCT eitems.ID, eitems.i_name, eitems.i_type FROM eitems INNER JOIN expensesm ON eitems.ID = expensesm.e_item WHERE expensesm.e_year='$the_year' AND expensesm.e_month='$the_month' AND i_type = 1") or die($conn->error());
			while ($row = $result->fetch_assoc()) { ?>
			<tr class="me">
			<?php
			$result2 = $conn->query("SELECT DISTINCT marakez.ID, marakez.mname FROM marakez INNER JOIN expensesm ON marakez.ID = expensesm.e_markaz WHERE expensesm.e_year='$the_year' AND expensesm.e_month='$the_month' ORDER BY ID DESC") or die($conn->error());
			while ($row2 = $result2->fetch_assoc()) {
			$result3 = $conn->query("SELECT * FROM expensesm WHERE e_year='$the_year' AND e_month='$the_month' AND e_markaz='".$row2['ID']."' AND e_item='".$row['ID']."'") or die($conn->error());
			$row3 = $result3->fetch_assoc(); ?>
			<td><?php echo number_format($row3['e_monthly']); ?></td>
			<td class="rig1">
				<?php if ($row['i_type'] != 1){ echo number_format($row3['e_num']); }?>
			</td>
			<?php } ?>
...
<div class="form-group">
	<br>
	<a href="expensesm.php?edit=<?php echo $the_year; ?>&edit2=<?php echo $the_month; ?>&r=3" class="cancel">تعديل</a>
	<a href="expensesm_report.php?y=<?php echo $the_year; ?>&m=<?php echo $the_month; ?>" class="cancel" target="_blank">طباعة</a>
