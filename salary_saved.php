...
<div class="row justify-content-center">
	<?php
	$result = $conn->query("SELECT DISTINCT marakez.ID, marakez.mname FROM marakez INNER JOIN salary ON marakez.ID = salary.s_markaz WHERE salary.s_year='$the_year' AND salary.s_month='$the_month' ORDER BY ID DESC") or die($conn->error());?>
	<table class="table cen1">
		...
		<?php 
			$result=$conn->query("SELECT DISTINCT sitems.ID, sitems.i_name FROM sitems INNER JOIN salary ON sitems.ID = salary.s_item WHERE salary.s_year='$the_year' AND salary.s_month='$the_month'") or die($conn->error());
			while ($row = $result->fetch_assoc()) { ?>
			<tr class="me">
			<?php
			$result2 = $conn->query("SELECT DISTINCT marakez.ID, marakez.mname FROM marakez INNER JOIN salary ON marakez.ID = salary.s_markaz WHERE salary.s_year='$the_year' AND salary.s_month='$the_month' ORDER BY ID DESC") or die($conn->error());
			while ($row2 = $result2->fetch_assoc()) {
			$result3 = $conn->query("SELECT * FROM salary WHERE s_year='$the_year' AND s_month='$the_month' AND s_markaz='".$row2['ID']."' AND s_item='".$row['ID']."'") or die($conn->error());
			$row3 = $result3->fetch_assoc(); ?>
			<td><?php echo number_format($row3['s_every']); ?></td>
			<td class="rig1"><?php echo number_format($row3['s_num']); ?></td>
			<?php } ?>
		...
		<div class="form-group">
			<br>
			<a href="salary.php?edit=<?php echo $the_year; ?>&edit2=<?php echo $the_month; ?>&r=3" class="cancel">تعديل</a>
			<a href="salary_report.php?y=<?php echo $the_year; ?>&m=<?php echo $the_month; ?>" class="cancel" target="_blank">طباعة</a>
