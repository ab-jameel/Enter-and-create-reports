...
<?php } endif;
	...
	$result = $conn->query("SELECT * FROM rok INNER JOIN months ON k_month = months.ID WHERE k_year = $the_year AND r_type = 5 ORDER BY k_month ASC") or die($conn->error());
?>
</div>
<br>
<div class="container">
...
<div class="row justify-content-center">
	<button class="save" style="margin-bottom: 20px; width: 250px; font-weight: bold;" onclick="window.location.href = 'expensesm.php';">ادخال مصروفات التأسيسية والمتقدمة<i class="fas fa-plus save1"></i></button>
</div>
...
<div class="row justify-content-center">
	...
	<table class="table right1">
		...
		<?php while ($row = $result->fetch_assoc()): ?>
			<tr class="me">
			<td class="right1">
				<a href="expensesm_view.php?m=<?php echo $row['k_month']; ?>&y=<?php echo $row['k_year']; ?>"><i class="fas fa-search i1"></i></a>
				<a href="expensesm_report.php?m=<?php echo $row['k_month']; ?>&y=<?php echo $row['k_year']; ?>" target="_blank"><i class="far fa-file-pdf i1"></i></a>
				<?php if($row['k_type'] == 0){ ?>
					<a href="expensesm.php?edit=<?php echo $row['k_year']; ?>&edit2=<?php echo $row['k_month']; ?>&r=4"><i class="fas fa-pencil-alt i1"></i></a>
				<?php } ?>
			</td>
